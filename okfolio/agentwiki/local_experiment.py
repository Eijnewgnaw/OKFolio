...
from .agentic import discover_from_headings
from .contracts import ConceptRef
from .global_cluster import build_clusters, candidate_edges
from .spatial_graph import build_graph_data, build_spatial_graph
from .versioning import reconcile_refs, semantic_key


def run(output: Path) -> str:
    """Run the public four-stage integration probe in ``output``."""
        ...
        judgements.append(
            {
                ...
                "relation_type": "supports" if related else "none",
                "direction": "bidirectional" if related else "none",
                "evidence_ref_ids": (
                    [edge.left_ref_id, edge.right_ref_id] if related else []
                ),
            }
        )
